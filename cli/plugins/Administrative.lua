  ...
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
	...
	if ((matches[1]:lower() == "sendfile" ) or (matches[1] == 'ارسال فایل' )) and matches[2] and matches[3] then
		...
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, send_file, litecyberch, dl_cb, nil)
	end
	if ((matches[1]:lower() == "sendplug" ) or (matches[1] == 'ارسال پلاگین' )) and matches[2] then
	    local plug = "./plugins/"..matches[2]..".lua"
		tdcli.sendDocument(msg.chat_id_, msg.id_,0, 1, nil, plug, litecyberch, dl_cb, nil)
    end
...
redis:set("litecyberLikes",2345)
redis:set("litecyberDisLikes",189)
return "انجام شد"
end
if msg.to.type ~= 'pv' then
...
if (matches[1]:lower() == 'litecyber' or matches[1] == 'ماتادور') and is_sudio(msg) then
local info_text = 
[[
》litecyber Cli v7.5
...
Info :]]..check_markdown(litecyberby)..[[

]]..check_markdown(litecyberch)..[[
]]
