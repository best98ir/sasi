...
----------------------------------------
local function delmsg (litecyber,sasan_cyber)
    msgs = litecyber.msgs 
    for k,v in pairs(sasan_cyber.messages_) do
        msgs = msgs - 1
        tdcli.deleteMessages(v.chat_id_,{[0] = v.id_}, dl_cb, cmd)
        if msgs == 1 then
            tdcli.deleteMessages(sasan_cyber.messages_[0].chat_id_,{[0] = sasan_cyber.messages_[0].id_}, dl_cb, cmd)
            return false
        end
    end
    tdcli.getChatHistory(sasan_cyber.messages_[0].chat_id_, sasan_cyber.messages_[0].id_,0 , 100, delmsg, {msgs=msgs})
end
----------------------------------------
...
if ((matches[1]:lower() == 'rmsg' ) or (matches[1] == 'پاکسازی' )) and is_mod(msg) then
	...
	--------------------------------
if ((matches[1]:lower() == "dev" ) or (matches[1] == "برنامه نویس" )) and is_sasan_cyber(msg) then 
